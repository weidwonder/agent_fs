...
当你已经在本地完成索引，并且希望把现有索引结果直接迁移到云端项目时，可以使用 `login` / `push` 子命令。
...
如果已经通过 package bin 安装了 CLI，也可以直接使用：

```bash
agent-fs login --target http://182.92.22.224:3000
agent-fs push --target http://182.92.22.224:3000 --project <云端项目 ID> "/path/to/local/project"
```

执行逻辑：

1. `login` 会调用云端 `/api/auth/login`，并将 CLI JWT 凭证写入 `~/.agent_fs/credentials.json`
2. `push` 会读取本地 `.fs_index`、AFD 归档和全局向量存储，逐文件调用云端 import API
3. 若本地与云端 embedding 模型一致，则直接迁移向量；否则只迁移切片与归档，由云端异步 re-embed
