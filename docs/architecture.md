...
说明：PDF 转换默认先使用 `pdfjs-dist` 逐页提取文本，并以“每页可提取字符数 `<100`（可配置）判为扫描页”的规则分类：全 text 直接输出、本地零 VLM；全 scan 回退到本地 `mineru-ts@1.0.3` 客户端；mixed 文档保留文本页直接提取结果，并用 MinerU 填充扫描页后按页合并。Agent FS 对 `Empty response from VLM server` 和瞬时网络异常保留整文件重试，并在重试时逐级降低 `maxConcurrency`（默认上限 4）；页面级并发、页级重试与失败页跳过由 `mineru-ts` 内建处理。
