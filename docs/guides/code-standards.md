...
- 适用于所有通过 `stdin/stdout` 与上游通信的进程，包括 MCP Server、插件子进程、JSON-RPC/自定义协议桥接进程。
- `stdout` 只能输出协议报文，禁止打印任何调试日志、启动横幅、第三方库提示、进度信息或 `console.log` 文本。
- 所有可观测性输出统一写入 `stderr`，例如启动成功提示、诊断日志、错误堆栈。
- 引入第三方库时，必须确认其不会默认向 `stdout` 打印内容；若库支持静默模式（如 `dotenv` 的 `quiet`），必须显式开启。
- 新增或修改 `stdio` 协议进程时，至少补一条验证：启动阶段在握手前不会向 `stdout` 写入非协议内容。
- 若出现“本地脚本可运行，但 MCP/协议客户端无法握手”的现象，优先排查是否有 `stdout` 污染。
